...
void XMLDocumentParser::doWrite(const String& parseString)
{
    ...
    if (parseString.length()) {
        ...
        // Hack around libxml2's error report about "XML declaration allowed only at the start of the document"
        // If we expected such error will happen, why not hack there?
        UChar* p_parseString = (UChar*)parseString.characters();
        unsigned start = 0;
        unsigned end = parseString.length() - 1;
        while (start <= end && isSpaceOrNewline(p_parseString[start]))
            start++;
        if(start > 0 && start < end) {
            xmlParseChunk(context->context(), reinterpret_cast<const char*>(parseString.characters() + start), sizeof(UChar) * (parseString.length() - start), 0);
        }else {
