...
void HTMLLinkElement::process()
{
    ...
    if (m_relAttribute.m_isDNSPrefetch) {
        ...
        if (settings && settings->dnsPrefetchingEnabled() && m_url.isValid() && !m_url.isEmpty())
	    	...
	    	ResourceHandle::prepareForURL(m_url, document()->frame()->view());
...
#if 1//ENABLE(LINK_PREFETCH)
    if ((m_relAttribute.m_isLinkPrefetch || m_relAttribute.m_isLinkPrerender || m_relAttribute.m_isLinkSubresource) && m_url.isValid()
                && document()->frame()) {
        if (!checkBeforeLoadEvent())
            return;
        ResourceLoadPriority priority = ResourceLoadPriorityUnresolved;
        ...
        //TODO prerender handler
        if(m_relAttribute.m_isLinkPrerender)
            document()->frame()->loader()->client()->dispatchDidLinkPrerender(m_url);
}
